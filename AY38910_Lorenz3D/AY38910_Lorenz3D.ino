...
void loop(){
  ...
  uint8_t notea = map(xout, 0, 127, 12, 72); // quantise note C1-c4
  uint8_t noteb = map(yout, 0, 127, 12, 72); // quantise note C1-c4
  uint8_t notec = map(zout, 0, 127, 12, 72); // quantise note C1-c4

  uint8_t vola = map(xout, 0, 127, 1, 13);
  uint8_t volb = map(yout, 0, 127, 1, 13);
  uint8_t volc = map(zout, 0, 127, 1, 13);
  ...
  if(drumon == true){
    ...
    noise(rand()%32); // 0-31
    set_env(1, 1, 1, 0); // hold, alter, attack, cont 0-1
  ...
  note_chan_A(notec-7); // note 0-127
  note_chan_B(notec+7); // note 0-127 
